...
const AnalyticsChart = ({ title, type = 'bar' }) => {
  ...
  const generateBlogGrowthData = () => {
    ...
    // Build cumulative but only up to current month — future months are null
    let cumulative = 0;
    return monthlyCount.map((count, i) => {
      if (i > currentMonth) return null; // don't show future months
      cumulative += count;
      return cumulative;
    });
  };
  ...
  const getMonthlyBlogCounts = () => {
    ...
    const monthlyCount = Array(12).fill(null);
    ...
    // Only return up to current month, past months with no blogs = 0
    return monthlyCount.map((val, i) => i <= currentMonth ? (val || 0) : null);
  };
  ...
  const maxVal = Math.max(...monthlyBlogData.filter(v => v !== null), 1);
  const maxGrowth = Math.max(...blogGrowthData.filter(v => v !== null), 1);
      ...
      <div className={wrapperClass}>
        ...
        <div className="space-y-4">
          {blogGrowthData.map((val, i) => {
            if (val === null) return null; // skip future months
            return (
              <div key={i} className="flex items-center gap-3">
                ...
                <div className={`flex-1 h-8 rounded-lg overflow-hidden ${isDark ? 'bg-dark-600' : 'bg-slate-100'}`}>
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: val > 0 ? `${(val / maxGrowth) * 100}%` : '0%' }}
      ...
      <div className={wrapperClass}>
        ...
        <div className="grid grid-cols-12 gap-2 h-40">
          {monthlyBlogData.map((val, i) => {
            if (val === null) return <div key={i} className="flex flex-col items-center justify-end gap-2 flex-1"><div className="w-full" /><span className={`text-xs ${isDark ? 'text-slate-700' : 'text-slate-300'}`}>{monthLabels[i]}</span></div>;
            return (
