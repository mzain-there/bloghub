...
import { FiCalendar, FiUser, FiTag, FiArrowLeft, FiHeart, FiEye } from 'react-icons/fi';
...
const BlogDetails = () => {
  ...
  const { getBlogById, allBlogs, loading, updateBlog } = useBlog();
  ...
  useEffect(() => {
    ...
    if (found) {
      ...
      // Increment view count only once when blog is first opened
      if (!viewIncrementedRef.current) {
        const newViewCount = (found.views || 0) + 1;
        updateBlog(found.id, { views: newViewCount });
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
          ...
          <LikeButton 
            ...
            onLikeChange={(newCount, isLiked) => {
              ...
              updateBlog(blog.id, { likes: newCount });
            }}
          />
        </motion.div>
